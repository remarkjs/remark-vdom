...
var visitors = compilers.visitors = {};
...
function generateFootnotes() {
    ...
    while (++index < length) {
        ...
        results[index] = self.visitors.listItem.call(self, {
            'type': 'listItem',
            'data': {
                'htmlAttributes': {
...
function one(node, parent) {
    ...
    return fn.call(self, node, parent);
}
...
function root(node) {
    ...
    return wrapInLines(self.all(node)).concat(self.generateFootnotes());
}
